...
router.post('/',
  ...
  body('capacity.current').isInt({ min: 0 }).optional().withMessage('Current capacity must be a non-negative integer'),
  body('capacity.maximum').isInt({ min: 1 }).withMessage('Maximum capacity must be a positive integer'),
...
router.put('/:id',
  ...
  body('capacity.current').isInt({ min: 0 }).optional().withMessage('Current capacity must be a non-negative integer'),
  body('capacity.maximum').isInt({ min: 1 }).optional().withMessage('Maximum capacity must be a positive integer'),
  body('instructor').isString().optional().withMessage('Instructor must be a string'),
  (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
